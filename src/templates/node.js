...
const NodePage = (props) => {
  ...
  const prevPage = currentPage - 1 === 1 ? '/category/node-js' : `/category/node-js/${currentPage - 1}`;
  const nextPage = `/category/node-js/${currentPage + 1}`;
...
export const pageQuery= graphql`
  query NodeQuery($skip: Int!, $limit: Int!) {
    allContentfulBlog(
